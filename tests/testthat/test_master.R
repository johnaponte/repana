...
test_that("The pipeline works fine", {
  ...
  expect_true(file.exists(file.path(curdir,"01_prog_a.R")))
  tmpfile <- tempfile()
  dir.create(tmpfile)
  setwd(tmpfile)
  ...
  dfres <- master()
  expect_true(file.exists("logs/01_prog_a.R.log"))
  expect_true(file.exists("logs/02_prog_b.R.log"))
  expect_true(file.exists("logs/03_prog_c.R.log"))
  expect_true(file.exists("logs/04_prog_d.R.log"))
...
test_that("Master respects the start & stop arguments", {
  ...
  expect_false(file.exists("logs/01_prog_a.R.log"))
  expect_true(file.exists("logs/02_prog_b.R.log"))
  expect_true(file.exists("logs/03_prog_c.R.log"))
  expect_false(file.exists("logs/04_prog_d.R.log"))
  expect_true(file.exists("logs/master.log"))
})
